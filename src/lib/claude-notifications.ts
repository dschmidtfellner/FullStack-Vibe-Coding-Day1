...
const CLAUDE_NOTIFICATION_ENDPOINT = 'https://us-central1-doulaconnect-messaging.cloudfunctions.net/sendClaudeNotification';
...
export async function sendClaudeNotification(
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Failed to send Claude notification:', response.status, response.statusText);
      return false;
    }
 ...
 */
export async function notifyWaitingForInput(message: string = 'Ready for your next instruction') {
 ...
 */
export async function notifyTaskCompleted(message: string = 'Task completed successfully') {
