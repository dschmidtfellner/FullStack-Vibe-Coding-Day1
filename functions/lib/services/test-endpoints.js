...
exports.exploreFCMTokenStorage = exports.sendClaudeNotification = exports.testPushNotification = void 0;
const admin = require("firebase-admin");
const fcm_tokens_1 = require("./fcm-tokens");
const push_notifications_1 = require("./push-notifications");
const firebase_apps_1 = require("./firebase-apps");
/**
 * Test push notification functionality
 */
async function testPushNotification(params, db) {
    ...
    if (userId && !fcmToken) {
        tokenToUse = await (0, fcm_tokens_1.getFCMTokenForUser)(userId, db);
        if (!tokenToUse) {
            throw new Error('No FCM token found for user');
        }
    }
    const success = await (0, push_notifications_1.sendPushNotification)(tokenToUse, title || 'Test Notification', body || 'This is a test message from the new Firebase messaging system');
    return {
        success,
 ...
 */
async function sendClaudeNotification(message, type) {
    if (!message || !type) {
    ...
    const result = await (0, push_notifications_1.sendOneSignalNotification)('rested', [davidPlayerId], `Claude Code - ${type}`, message);
...
exports.exploreFCMTokenStorage = exploreFCMTokenStorage;
/**
