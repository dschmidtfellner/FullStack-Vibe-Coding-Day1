...
async function sendOneSignalNotificationToAllApps(
  playerIds: { rested?: string[]; doulaConnect?: string[] },
  title: string,
  body: string,
...
async function getPlayerIdsForUser(userId: string): Promise<{ rested?: string[]; doulaConnect?: string[] }> {
  try {
...
async function sendPushNotificationsForMessage(message: any, notificationData: {
  recipients: string[];
  senderName: string;
...
async function getChildPushRecipientsAndSenderInfo(childId: string, senderId: string): Promise<{
  recipients: string[];
  senderName: string;
  primaryCaregiverId: string;
  altOrg: string;
