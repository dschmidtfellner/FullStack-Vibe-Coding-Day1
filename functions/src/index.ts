...
export const getUnreadCount = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const count = unreadCountDoc.exists ? unreadCountDoc.data()?.count || 0 : 0;
...
export const getAllUnreadCounts = functions.https.onRequest(async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, POST');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
...
export const incrementUnreadCount = functions.firestore
